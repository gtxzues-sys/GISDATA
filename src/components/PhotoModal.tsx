...
+import { X, Calendar, Tag, Palette, Shapes, Trash2 } from 'lucide-react';
...
+export default function PhotoModal({ sign, onClose, onDelete }: PhotoModalProps) {
...
+
+    try {
+      const result = await onDelete(sign.id);
+      if (result.success) {
+        onClose();
+      } else {
